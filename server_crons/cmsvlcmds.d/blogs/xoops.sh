...
cmshomepage='http://xoops.org';
cmslatestvurl='http://xoops.org/modules/core/';
cmsdescrip='NA';
...
cmslatestversion=$(curl $curlargs $cmslatestvurl |grep -Po "XOOPS (\d{1})\.(\d{1,2})\.(\d{1,2})?\.(\d{1,2})" |sed 's/XOOPS //g');
