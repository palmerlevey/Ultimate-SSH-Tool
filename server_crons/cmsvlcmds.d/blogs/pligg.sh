...
cmshomepage='http://pligg.com';
cmslatestvurl='http://pligg.com/downloads/package/pligg-cms/';
cmsdescrip='NA';
...
cmslatestversion=$(curl $curlargs $cmslatestvurl |grep -Po "Product Version (\d{1})\.(\d{1,2})\.(\d{1,2})" |sed 's/Product Version //g');
